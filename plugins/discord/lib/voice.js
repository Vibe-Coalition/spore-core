...
function loadVoicePackages(log = console) {
  if (voiceModule && prismMedia) return true;
  if (voiceLoadAttempted) return false;
  voiceLoadAttempted = true;
  try {
    voiceModule = requireHostPackage('@discordjs/voice');
    prismMedia = requireHostPackage('prism-media');
    return true;
  } catch (e) {
    log.warn?.('[discord-voice] require failed: ' + e.message);
    return false;
  }
}
...
class DiscordVoice {
  ...
  _ensureVoicePipeline() {
    ...
    try {
      const { VoicePipeline } = coreRequire('voice');
  ...
  async handleJoin(message) {
    ...
    if (!pipeline?.enabled) {
      await message.reply('Voice pipeline not configured. Open Settings → Voice to install an STT plugin and configure a TTS provider.');
      return;
    }
