...
let SlackBolt = null;
function loadSlackBolt() {
  if (!SlackBolt) SlackBolt = requireHostPackage('@slack/bolt');
  return SlackBolt;
}
...
class SlackGateway {
  ...
  _loadGateConfig() {
    try {
      const raw = require('fs').readFileSync(
        require('path').join(this.config.dataDir || process.cwd(), 'spore.json'), 'utf8'
      );
  ...
  _sessionChannelId(channelId, threadTs = null) {
    const mode = this.config.channels?.slack?.sessionMode || 'channel';
    return mode === 'thread' && threadTs ? `${channelId}:thread:${threadTs}` : channelId;
  }

  _sessionKey(channelId, isDm, userId, threadTs = null, policy = null) {
    return this.agent.sessions.constructor.buildKey({
      platform: 'slack',
      channelId: this._sessionChannelId(channelId, threadTs),
      isDm,
      userId,
  ...
  async connect() {
    ...
    if (!slackCfg.enabled || !botToken || !appToken) {
      this.log.info('[slack] Gateway disabled — bot token/app token not configured');
      return null;
    }

    const { App } = loadSlackBolt();
    this.app = new App({
      token: botToken,
      appToken,
      socketMode: true,
  ...
  async _onMessage(message, say, client) {
    ...
    if (trigger && policy.respond) {
      ...
      this._queue.scheduleLull(channelId, () => {
        this._maybeLullResponse(channelId, channelName, isDm, userId, userName, client, say, threadTs, sessionKey);
      });
    }
  }
  ...
  async _maybeLullResponse(channelId, channelName, isDm, userId, userName, client, say, threadTs, sessionKey = null) {
  ...
  async _handleNewSession(command, say) {
    ...
    try {
      const sessionKey = this._sessionKey(channelId, false, userId, command.thread_ts || command.message_ts || null);
  ...
  async _handleReload(command, say) {
    await say('Channel plugins are hot-reloaded from Settings -> Plugins. Restart this channel plugin from there or restart the instance.');
  }
  ...
  getStatus() {
    ...
    const botToken = slackCfg.botToken || this.config.slackBotToken;
    const appToken = slackCfg.appToken || this.config.slackAppToken;
    if (!slackCfg.enabled || !botToken || !appToken) return 'disabled';
