...
The JSON schema for updates becomes:
  ...
  _writeToGraph(extraction, opts = {}) {
    ...
    // other than the human currently talking. Without this, a test-user→tim
    // conversation lets the LLM attach test-user's facts (Windows path, OS,
    ...
    try {
      ...
      for (const upd of (extraction.updates || [])) {
        ...
        // Same cross-user guard as the aspects loop — never let test-user's
