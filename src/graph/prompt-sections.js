...
- **Rules**:
  ...
  proto._buildRuntimeSection = function _buildRuntimeSection(opts) {
    ...
    if (opts.projectContext) {
      ...
      parts.push('**Scratch scripts go in `.acorn/scratch/`, NEVER the project root.** For ad-hoc helpers (LAN IP detection, QR generation, one-off test runners, log scrapers, any `_get-X.js`/`tmp-Y.py` style file that isn\'t part of the project itself), write to `.acorn/scratch/` — e.g. `.acorn/scratch/get-lan-ip.js`, `.acorn/scratch/gen-qr.js`. The `.acorn/` directory already holds plans and logs; it\'s the designated acorn workspace and is typically gitignored. Writing `_qr4.js` / `_get-lan-ip3.js` / etc. to the project root pollutes the user\'s repo and clutters their file listing. Before writing a scratch file, check if a prior session has the same helper — `graph_query` the tool/library node first; reuse + adapt beats rewriting. When a scratch script does something non-obvious and durable, record the approach (not the full source) on the relevant framework/library node so future sessions can reproduce without re-deriving.');
