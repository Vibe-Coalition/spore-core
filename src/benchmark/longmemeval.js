...
class LongMemEvalRunner {
  ...
  async run(opts = {}) {
    ...
    const maxQuestions = opts.maxQuestions || 500;
    ...
    this.log.info(`[longmemeval] Starting benchmark: variant=${variant} maxQ=${maxQuestions} skip=${skipIngestion} fresh=${forceReeval} types=${questionTypes ? questionTypes.join(',') : 'all'}`);
    ...
    try {
      this.broadcast({ type: 'benchmark:start', variant, maxQuestions });

      // Load dataset
      const dataset = await this._loadDataset(variant);

      // Filter by question type if specified, then stratified sample
      let filtered = dataset;
      if (questionTypes && questionTypes.length > 0) {
        const typeSet = new Set(questionTypes);
        filtered = dataset.filter(q => typeSet.has(q.question_type));
      }
      ...
      if (filtered.length <= maxQuestions) {
        ...
        // Stratified sampling: proportional picks from each type
        const byType = {};
        ...
        questions = [];
        const perType = Math.max(1, Math.floor(maxQuestions / types.length));
      ...
      this.broadcast({
        ...
        message: `Loaded ${questions.length} questions (${variant} variant)`,
      });
...
module.exports = { LongMemEvalRunner };
