...
class DiscordGateway {
  ...
  _onReady() {
    ...
    this.client.user.setPresence({
      activities: [{ name: 'the graph', type: ActivityType.Watching }],
      status: 'online',
    });
  }
  ...
  /**
   * /reload — Hot-reload tools.js and context.js without restarting the gateway.
   * Clears Node's require cache for the target modules and re-instantiates them.
   * Discord connection, sessions, and agent loop stay untouched.
   */
  async _handleReload(message) {
    try {
      ...
      // Re-instantiate tools with fresh code
      ...
      freshConfig.discordToken = this.config.discordToken;
      freshConfig.anthropicApiKey = this.config.anthropicApiKey;
      freshConfig._isOAuth = this.config._isOAuth;
      Object.assign(this.config, freshConfig);

      // Rebuild graph context with fresh config
      ...
      // Rebuild tool system
      const tools = new ToolSystem(this.config, this.log, this.client, graph, this.agent.client);
      tools.discord = this.client;
      tools.discordGateway = this;
      ...
      // Swap AgentLoop prototype — hot-patches agent.js logic without losing session state
      const { AgentLoop } = require('../agent');
      Object.setPrototypeOf(this.agent, AgentLoop.prototype);

      // Swap DiscordGateway prototype — hot-patches discord.js methods without dropping connection
      const { DiscordGateway } = require('./discord.js');
      Object.setPrototypeOf(this, DiscordGateway.prototype);

      // Reload gate config
      this._loadGateConfig();

      await message.reply(`🔄 Hot-reloaded: ${reloaded.join(', ')}. Config, tools, context, and agent logic refreshed.`);
      this.log.info(`[reload] Hot-reloaded modules: ${reloaded.join(', ')}`);
    } catch (e) {
      await message.reply(`❌ Reload failed: ${e.message}`).catch(() => {});
      this.log.error('[reload] Failed:', e.message, e.stack);
    }
  }
