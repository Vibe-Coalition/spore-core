-- Idempotent migration: ref-email node documenting the email tools.
-- Run on every boot; WHERE NOT EXISTS guards each aspect / attribute insert.
...
VALUES ('ref-email', 'Email (send + read)', 'reference',
  'The agent has its own mailbox when the operator wires up an email provider in Settings → Agent → Email. Exposes four tools for sending and reading mail via SMTP + IMAP. Proton is supported first; Google is planned.', 9, 'seed');

INSERT INTO aspects (node_id, name, weight, extracted_with)
  SELECT 'ref-email', 'overview', 10, 'seed'
...
INSERT INTO attributes (aspect_id, content, importance, source, extracted_with)
  SELECT (SELECT id FROM aspects WHERE node_id='ref-email' AND name='overview'),
         'Proton uses their SMTP submission service (smtp.protonmail.ch:587 + imap.protonmail.ch:993) with an SMTP token generated in Proton settings (requires a paid Proton Mail plan). The operator manages the credentials in the Agent settings pane.',
         8, 'seed', 'seed'
  WHERE EXISTS (SELECT 1 FROM aspects WHERE node_id='ref-email' AND name='overview')
...
INSERT INTO aspects (node_id, name, weight, extracted_with)
  SELECT 'ref-email', 'troubleshooting', 7, 'seed'
...
-- Edge connecting ref-email to the agent's self node via `documents`
INSERT INTO edges (source, target, type, weight, extracted_with)
  SELECT 'spore', 'ref-email', 'documents', 0.8, 'seed'
