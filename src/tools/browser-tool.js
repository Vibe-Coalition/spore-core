...
class BrowserTool {
  ...
  _isBlockedUrl(urlStr) {
    try {
      const parsed = new URL(urlStr);
      if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') return `Blocked: only http/https URLs allowed (got ${parsed.protocol})`;
      if (_blockedHostPatterns.some(r => r.test(parsed.hostname))) return `Blocked: access to ${parsed.hostname} is not allowed (private/internal network)`;
      return null;
    } catch {
  ...
  async _launch(input) {
    ...
    if (url !== 'about:blank') {
      const blocked = this._isBlockedUrl(url);
    ...
    try {
      ...
      this.log.info(`[browser] Launched — ${url} (${width}x${height}), screencast active`);
  ...
  async _startScreencast() {
    ...
    try {
      ...
      this.log.warn(`[browser] Screencast start failed: ${e.message}`);
    }
  }
  ...
  _broadcastFrame(header, jpegBuffer) {
    ...
    const frame = Buffer.concat([lenBuf, headerBuf, jpegBuffer]);
    this._broadcast(frame, true);
  }
  ...
  async _navigate(input) {
    ...
    try {
      await this._page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });
      const title = await this._page.title();
      this.log.info(`[browser] Navigate — ${url} (${title})`);
  ...
  async _screenshot() {
    ...
    try {
      ...
      return {
        ...
        message: 'High-quality screenshot sent to the control panel.',
      };
    } catch (e) {
  ...
  async _evaluate(input) {
    ...
    try {
      ...
      return {
        status: 'evaluated',
        result: str && str.length > 4000 ? str.substring(0, 4000) + '...' : result,
      };
    } catch (e) {
